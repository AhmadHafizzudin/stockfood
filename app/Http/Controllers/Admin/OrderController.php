...
class OrderController extends Controller
{
    ...
    public function status(Request $request)
    {
        ...
        // Auto-dispatch via Lalamove only when order is handed over
    ...
    /**
     * Attempt to dispatch the order via Lalamove (MY) on status change.
     */
    private function dispatchViaLalamove(\App\Models\Order $order): void
    {
        try {
            ...
            if (!empty($order->lalamove_order_id)) {
                \Log::info('Lalamove dispatch skipped: already has order ID', ['order_id' => $order->id, 'lalamove_order_id' => $order->lalamove_order_id]);
                return;
            }
            ...
            if (!$lalamoveConfigured) {
                \Log::info('Lalamove dispatch skipped: not configured in env', ['order_id' => $order->id]);
                return;
            }
            ...
            if (!$restaurant || !$delivery || !isset($delivery['latitude'], $delivery['longitude'])) {
                \Log::warning('Lalamove dispatch aborted: missing location data', [
                    'order_id' => $order->id,
            ...
            if ($url = config('lalamove.webhook_url')) {
                ...
                if (!($wh['success'] ?? false)) {
                    \Log::warning('Lalamove webhook update failed', ['order_id' => $order->id, 'error' => $wh['error'] ?? null]);
                }
            }
            ...
            if (!$quote['success']) {
                \Log::error('Lalamove quotation failed', ['order_id' => $order->id, 'error' => $quote['error'] ?? null]);
                return;
            }
            ...
            if ($create['success']) {
                ...
                if ($lm) {
                    ...
                    $order->save();
                    \Log::info('Lalamove order created', ['order_id' => $order->id, 'lalamove_order_id' => $lm]);
                }
            } else {
                \Log::error('Lalamove create order failed', ['order_id' => $order->id, 'error' => $create['error'] ?? null]);
            }
        } catch (\Throwable $e) {
            \Log::error('Lalamove dispatch exception', ['order_id' => $order->id, 'message' => $e->getMessage()]);
        }
    }
