...
class ConfigController extends Controller
{
    ...
    public function configuration()
    {
        ...
        return response()->json([
            ...
            'customer_wallet_status' => (int)(isset($settings['wallet_status']) ? (is_numeric($settings['wallet_status']) ? (int)$settings['wallet_status'] : ((json_decode($settings['wallet_status'], true)['status'] ?? 0))) : 0),
            ...
            'add_fund_status' => (int)(isset($settings['add_fund_status']) ? (is_numeric($settings['add_fund_status']) ? (int)$settings['add_fund_status'] : ((json_decode($settings['add_fund_status'], true)['status'] ?? 0))) : 0),
    ...
    private function getDefaultPaymentMethods()
    {
        ...
        $methods = DB::table('addon_settings')->where('is_active',1)->whereIn('settings_type', ['payment_config'])->whereIn('key_name', ['ssl_commerz','paypal','stripe','razor_pay','senang_pay','zenpay','paytabs','paystack','paymob_accept','paytm','flutterwave','liqpay','bkash','mercadopago'])->get();
        $env = env('APP_ENV') == 'live' ? 'live' : 'test';
        $credentials = $env . '_values';
